...
export default function TarotFlow() {
  ...
  return (
    <main className="min-h-screen max-w-md mx-auto px-4 pt-6 pb-28">
      ...
      <h1 className="mt-4 text-3xl font-serif text-violet-100">塔罗占卜</h1>

      {step === 'setup' ? (
        <section className="mt-6 space-y-4">
          <div className="rounded-2xl border border-zinc-700 bg-zinc-900/70 p-4">
            <div className="text-sm text-zinc-300 mb-2">占卜问题</div>
            <textarea
              value={question}
          ...
          <div className="rounded-2xl border border-zinc-700 bg-zinc-900/70 p-4">
            <div className="text-sm text-zinc-300 mb-2">牌阵模式</div>
            <div className="space-y-2">
              {spreads.map((item) => (
                <button
                  key={item.id}
                  className={`w-full rounded-xl border p-3 text-left ${spread === item.id ? 'border-violet-300 bg-violet-950/40' : 'border-zinc-700 bg-zinc-950'}`}
                  onClick={() => setSpread(item.id)}
                >
                  <div className="text-violet-100">{item.label}</div>
                  <div className="text-xs text-zinc-400">{item.desc} · {item.count} 张</div>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-zinc-700 bg-zinc-900/70 p-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-sm text-zinc-200">逆位设置</div>
                <div className="text-xs text-zinc-400">开启后会出现正逆位解读</div>
              </div>
              <button
                onClick={() => setReverseEnabled((v) => !v)}
                className={`h-8 w-14 rounded-full border ${reverseEnabled ? 'border-violet-300 bg-violet-500/40' : 'border-zinc-600 bg-zinc-800'}`}
              >
                <div className={`h-6 w-6 rounded-full bg-white transition ${reverseEnabled ? 'translate-x-7' : 'translate-x-1'}`} />
              </button>
            </div>
          </div>

          <button disabled={loading} onClick={() => void begin()} className="w-full py-3 rounded-xl bg-violet-600 text-white">
            {loading ? '准备中...' : '进入抽牌'}
          </button>
        </section>
      ) : null}
      ...
      {step === 'draw' ? (
        <section className="mt-6 rounded-2xl border border-violet-300/30 bg-zinc-900/70 p-4">
          <div className="text-sm text-zinc-300 mb-3">从牌堆抽牌：{selected.length}/{requiredCount}</div>
          <div className="grid grid-cols-5 gap-2">
            {Array.from({ length: 20 }).map((_, idx) => (
              <button
                ...
                className="h-16 rounded-lg border border-violet-300/30 bg-gradient-to-b from-violet-950 to-zinc-950 text-[10px] text-violet-100/70"
              >
                牌堆
              </button>
            ))}
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {selected.map((item, idx) => (
                <div key={`${item.cardId}-${idx}`}>
                  <div className="text-[10px] text-zinc-400 mb-1">{item.position}</div>
          ...
          <button
            ...
            className="mt-4 w-full py-3 rounded-xl bg-amber-600 text-white disabled:opacity-40"
          >
            分析牌阵
          </button>
        </section>
      ...
      {step === 'result' ? (
        <section className="mt-6 space-y-4">
          ...
          {recordId ? (
            <div className="rounded-2xl border border-zinc-700 bg-zinc-900/70 p-4">
              <div className="text-sm text-zinc-300 mb-2">单张/整体追问</div>
              <div className="flex gap-2">
                ...
                />
                <button onClick={() => void askFollowup()} className="px-3 py-2 rounded-lg bg-violet-600">发送</button>
              </div>
