...
interface LiuyaoShakeStepProps {
  manualLines: number[];
  lastCoins: number[];
  casting: boolean;
  loading: boolean;
...
function renderYao(line: number, moving: boolean, key: string) {
  ...
  return (
    <div key={key} className="relative">
      {isYang ? (
        <div className={`h-3 w-full rounded-sm ${moving ? 'bg-[#52B788] animate-pulse' : 'bg-[#171817]'}`} />
      ) : (
...
export default function LiuyaoShakeStep({
  manualLines,
  lastCoins,
  casting,
  loading,
  onShake,
  onGenerate,
}: LiuyaoShakeStepProps) {
  const progress = manualLines.length;
  const previewLines = useMemo(() => [...manualLines].reverse(), [manualLines]);

  return (
    <motion.section
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="mt-6 rounded-3xl border border-[#171817]/10 bg-white/80 p-5 md:p-8"
    >
      <div className="text-center mb-6">
        <p className="text-sm tracking-[0.3em] text-[#171817]/55 font-medium">点击龟壳投掷铜钱</p>
        <p className="text-xs text-[#171817]/40 tracking-widest mt-2">
          已投掷 <span className="text-[#52B788] font-bold">{progress}</span> / 6
        </p>
      </div>

      <div className="relative w-72 h-72 md:w-96 md:h-96 flex items-center justify-center mb-8 mx-auto">
        ...
        >
          <motion.div
            animate={{
              rotate: casting ? [0, -8, 8, -5, 0] : [0, -3, 3, -3, 0],
              x: casting ? [0, -6, 6, -4, 0] : [0, -1, 1, -1, 0],
            }}
          ...
          >
            ...
            <div
              className="absolute inset-0 opacity-25"
              style={{ background: 'radial-gradient(circle at 50% 50%, transparent 0 38%, rgba(24,14,10,0.55) 76%, rgba(12,8,6,0.85) 100%)' }}
            />
        ...
        <div className="absolute inset-0 z-20 pointer-events-none flex items-center justify-center">
          {(lastCoins.length ? lastCoins : [2, 3, 2]).map((coin, i) => (
            <motion.div
              key={`${coin}-${i}-${progress}`}
              ...
              style={{
                background: 'radial-gradient(circle at 32% 28%, #f5d993 0%, #d8a24d 45%, #9a6525 100%)',
              }}
            >
      ...
      <div className="flex flex-col items-center space-y-6 w-full max-w-md mx-auto">
        <div className="text-center">
          ...
          <h2 className="text-3xl font-bold tracking-widest text-[#171817]">{progress === 6 ? '卦象已成' : `第 ${progress + 1} 次`}</h2>
        </div>

        <div className="w-full">
          <div className="text-xs text-[#171817]/40 mb-3 text-center font-medium">
            进度 <span className="text-[#52B788]">{progress}</span> / 6
          </div>
            ...
            {[...Array(6)].map((_, i) => (
              <div
                ...
                className={`h-1.5 flex-1 transition-all duration-300 rounded-full ${
                  i < progress ? 'bg-[#52B788] shadow-md' : 'bg-[#171817]/10'
                }`}
              />
            ))}
          </div>
        ...
        <button
          disabled={progress !== 6 || loading}
          onClick={onGenerate}
          className="px-10 py-3.5 bg-[#52B788] text-white rounded-full font-bold tracking-widest hover:bg-[#40916C] hover:scale-105 transition-all shadow-lg disabled:opacity-45 disabled:hover:scale-100"
        >
          查看解读
        </button>
      </div>
